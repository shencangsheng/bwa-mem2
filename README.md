...
## Getting Started
```sh
# Use the CentOS 7-compatible precompiled package (recommended)
...
[release page][rel]. Each release ships one CentOS 7-compatible Linux tarball:

| File | OS / glibc | Root |
|------|------------|------|
| `bwa-mem2-<tag>_centos7_x64-linux.tar.bz2` | CentOS 7 / RHEL 7+ (glibc 2.17) | Official Intel + GCC fallback |

```
bwa-mem2-<tag>_centos7_x64-linux/
  bwa-mem2                        # CentOS 7-compatible super-dispatcher
  bwa-mem2.sse41 … .avx512bw      # upstream v2.2.1 Intel ISA siblings
  extra/
    bwa-mem2.gcc-full             # AMD / Intel no-CLWB portable handoff
    bwa-mem2.gcc-avx2-fleet
    bwa-mem2.sse41 … bwa-mem2.avx512bw
```

- **Root `bwa-mem2`** uses the official Intel siblings on compatible Intel CPUs.
  AMD EPYC and Intel AVX-512 CPUs without CLWB hand off to
  `extra/bwa-mem2.gcc-full` ([#160](https://github.com/bwa-mem2/bwa-mem2/issues/160),
  [#236](https://github.com/bwa-mem2/bwa-mem2/issues/236)).
- **All binaries** require at most glibc 2.17 and run on CentOS 7.9 or newer.
- **`extra/`** holds portable/pinned GCC builds; normally do not call them directly.
